...
public class NetworkDiskServiceImpl extends SuperServiceImpl<NetworkDiskMapper, NetworkDisk> implements NetworkDiskService {
    ...
    public List<NetworkDisk> selectFileList(NetworkDisk networkDisk) {
        ...
        // 获取配置文件
        Map<String, String> systemConfigMap = feignUtil.getSystemConfigMap(request.getAttribute(SysConf.TOKEN).toString());
        ...
        for (NetworkDisk item: list) {
            if (EFilePriority.QI_NIU.equals(picturePriority)) {
                item.setFileUrl(systemConfigMap.get(SysConf.QI_NIU_PICTURE_BASE_URL) + item.getQiNiuUrl());
            } else if (EFilePriority.MINIO.equals(picturePriority)) {
                item.setFileUrl(systemConfigMap.get(SysConf.MINIO_PICTURE_BASE_URL) + item.getMinioUrl());
            } else if (EFilePriority.ALIOSS.equals(picturePriority)) {
                item.setFileUrl(systemConfigMap.get(SysConf.ALI_OSS_PICTURE_BASE_URL) + item.getAliOssUrl());
            } else {
                item.setFileUrl(systemConfigMap.get(SysConf.LOCAL_PICTURE_BASE_URL) + item.getLocalUrl());
            }
        }
        return list;
    }
    ...
    public void deleteFile(NetworkDiskVO networkDiskVO, Map<String, String> qiNiuConfig) {
        ...
        String uploadAlioss = qiNiuConfig.get(SysConf.UPLOAD_ALIOSS);
        // 获取系统配置文件
        ...
        if (SysConf.ONE == networkDisk.getIsDir()) {
            ...
            if (list.size() > 0) {
                ...
                if (isUpdateSuccess) {
                    ...
                    if (EOpenStatus.OPEN.equals(uploadMinio)) {
                        ...
                        minioUtil.deleteBatchFile(systemConfig, fileList);
                    }
                    //删除阿里OSS中的文件
                    if (EOpenStatus.OPEN.equals(uploadAlioss)) {
                        List<String> fileList = new ArrayList<>();
                        list.forEach(item -> {
                            fileList.add(item.getAliOssUrl());
                        });
                        aliossUtil.deleteBatchFile(systemConfig, fileList);
                    }
                }
            }
        } else {
            ...
            //删除阿里oss中的文件
            if (EOpenStatus.OPEN.equals(uploadAlioss)) {
                String aliossUrl = networkDisk.getAliOssUrl();
                if (StringUtils.isNotEmpty(aliossUrl)) {
                    String[] minUrlArray = aliossUrl.split("/");
                    // 找到文件名
                    aliossUtil.deleteFile(minUrlArray[minUrlArray.length - 1], systemConfig);
                } else {
                    log.error("删除的文件不存在阿里oss文件地址");
                }
            }
            ...
            if (EOpenStatus.OPEN.equals(uploadMinio)) {
                ...
                if (StringUtils.isNotEmpty(minioUrl)) {
                    ...
                    // 找到文件名
                    minioUtil.deleteFile(systemConfig, minUrlArray[minUrlArray.length - 1]);
                } else {
