...
<script>
  ...
  import {getMediaPlayRecord, insertOrUpdate} from "../../api/mediaPlayRecord";
  ...
  export default {
    ...
    mounted() {
      const queryPlayParam = { videoUid: this.videoId }
      getMediaPlayRecord(queryPlayParam).then(response => {
    ...
    methods: {
      ...
      insertPlayLog() {
       const insertPlayParam = { videoUid: this.videoId, playDuration: this.duration, playPosition:  this.player.cache_.currentTime }
          insertOrUpdate(insertPlayParam).then(response => {
        })
      },
      //点击视频播放
      play(player){
      ...
      playerReadied(player) {
        let openMediaPlay = this.videoPreviewList[this.activeIndex].openMediaPlay
        if (openMediaPlay) {
