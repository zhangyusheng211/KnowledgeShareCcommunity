<template>
  <div style="margin: 0 auto">
    ...
    <span class="left" v-if="isShowAvatar">
      ...
      <span class="right" :class="isShowAvatar?'rightWidth':'rightMaxWidth'">
        <div class="chat-input-box-container">
                <div @paste.capture.prevent="handlePaste" v-on="listeners" v-model="inputContent" class="chat-input-box"
                     ref="inputBox" placeholder="既然来了，那就留下些什么吧~ 【支持Markdown语法】"
    ...
    <div class="bottom">
      ...
      <emoji class="cancel p2" @selected="emojiSelected" @sendPic="sendPic" bottomIndex="53px"></emoji>
...
export default {
  ...
  methods: {
      ...
      if (matchs) {
        ...
        if (imgKeys.length > 0) {
          _this.uploadPasteImages(imgKeys).then((res) => {
              ...
              console.log("提交评论", comments)
              _this.$emit("submit-box", comments)
              _this.inputContent = ""
              _this.$refs.inputBox.innerHTML = ""
            },
            (err) => {
              console.error(err)
              _this.$notify.error({
        ...
        _this.$emit("submit-box", this.comments)
        _this.inputContent = ""
        _this.$refs.inputBox.innerHTML = ""
      }
    },
    ...
    sendPic(url, fileUid) {
      let imgTag = `<img class="chat-msg-img" src="${url}" style="max-height: 100px"  uid="${fileUid}">`;
      this.inputContent = this.inputContent + imgTag
      this.$refs.inputBox.innerHTML = this.inputContent
      this.$commonUtil.moveCursorToEnd(this.$refs.inputBox, false)
    },
