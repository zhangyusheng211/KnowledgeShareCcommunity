    ...
    <div v-for="(item, index) in comments" :key="item.uid" >
      <div class="commentList">
        <span class="left p1" :class="item.user.userTag > 0 ?'vip-avatar':''">
            <img :class="item.user.userTag > 0 ?'vip-color':''" v-if="item.user" :src="item.user.photoUrl ? item.user.photoUrl:defaultAvatar" onerror="onerror=null;src=defaultAvatar" />
            <img v-else :src="defaultAvatar" />
            <span style="font-size: 10px; right: -7px; bottom: -2px;" class="vip-text pointer" v-if="item.user.userTag > 0">v</span>
        </span>
