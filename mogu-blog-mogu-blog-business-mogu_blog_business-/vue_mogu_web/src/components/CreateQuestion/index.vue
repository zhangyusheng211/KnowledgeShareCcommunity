    ...
    >
      <el-form :model="form" :rules="rules" ref="form">
        ...
        <el-row>
          ...
          <el-col :span="6.5">
            <el-form-item label="问答模板" :label-width="formLabelWidth" prop="questionTemplateUid">
              <el-select
...
<!--          <MarkdownEditor ref="editor" v-if="editorModel == '1'" :content="form.content"  :height="560"></MarkdownEditor>-->
          <mavon ref="editor" v-if="editorModel == '1'" :context="form.content" :height="560"></mavon>
          <CKEditor ref="editor" v-else :content="form.content" :height="500"></CKEditor>
        </el-form-item>
          ...
          if (this.isEdit) {
            editQuestion(this.form).then(response => {
              if (response.code == this.$ECode.SUCCESS) {
                ...
                setTimeout(()=> {
                  // 跳转到我的问答
                  window.location.href = this.$commonUtil.setUrlParams("action", "MyQuestion")
                }, 500)
              } else {
