...
public class SubjectItemServiceImpl extends SuperServiceImpl<SubjectItemMapper, SubjectItem> implements SubjectItemService {
    ...
    public IPage<SubjectItem> getPageList(SubjectItemVO subjectItemVO) {
        ...
        if (StringUtils.isNotEmpty(subjectItemVO.getIsPublish())) {
            queryWrapper.eq(SQLConf.IS_PUBLISH, subjectItemVO.getIsPublish());
        }
        ...
        if (blogUidList.size() > 0) {
            ...
            if (blogCollection.size() > 0) {
                ...
                blogList.forEach(item -> {
                    ...
                    item.setContent("");
                    // 门户仅显示已发布的博客
                    if (EPublish.PUBLISH.equals(subjectItemVO.getIsPublish()) && EPublish.PUBLISH.equals(item.getIsPublish())) {
    ...
    public String addSubjectItemList(List<SubjectItemVO> subjectItemVOList) {
        ...
        for (SubjectItemVO subjectItemVO : subjectItemVOList) {
            ...
            if (!repeatBlogList.contains(subjectItemVO.getBlogUid())) {
                ...
                subjectItem.setTryRead(subjectItemVO.getTryRead());
        ...
        if (subjectItemUidList.size() > 0) {
            ...
            if (subjectItemCollection.size() > 0) {
                ...
                subjectItemCollection.forEach(item -> {
                    ...
                    item.setTryRead(subjectItemVO.getTryRead());
    ...
    @Override
    public Map<String, Integer> getCountBySubjectUidList(List<String> subjectUidList) {
        Map<String, Integer> resultMap = new HashMap<>();
        if (subjectUidList.size() == 0) {
            return resultMap;
        }
        QueryWrapper<SubjectItem> queryWrapper = new QueryWrapper<>();
        queryWrapper.select("subject_uid, count(*) as count");
        queryWrapper.in(SQLConf.SUBJECT_UID, subjectUidList);
        queryWrapper.eq(SQLConf.STATUS, EStatus.ENABLE);
        queryWrapper.groupBy(SQLConf.SUBJECT_UID);

        List<Map<String, Object>> listMaps = subjectItemService.listMaps(queryWrapper);

        for (Map<String, Object> map : listMaps) {
            if (map.get(SQLConf.SUBJECT_UID) != null && map.get(SQLConf.COUNT) != null) {
                String subjectUid = map.get(SQLConf.SUBJECT_UID).toString();
                Integer count = Integer.valueOf(map.get(SQLConf.COUNT).toString());
                resultMap.put(subjectUid, count);
            }
