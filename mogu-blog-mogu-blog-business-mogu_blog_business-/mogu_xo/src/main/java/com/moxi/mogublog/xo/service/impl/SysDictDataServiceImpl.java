...
public class SysDictDataServiceImpl extends SuperServiceImpl<SysDictDataMapper, SysDictData> implements SysDictDataService {
    ...
    public Map<String, Object> getListByDictType(String dictType) {
        ...
        redisUtil.setEx(SysConf.REDIS_DICT_TYPE + SysConf.REDIS_SEGMENTATION + dictType, JsonUtils.objectToJson(result), 1, TimeUnit.DAYS);
        return result;
    }
    ...
    public Map<String, Map<String, Object>> getListByDictTypeList(List<String> dictTypeList) {
        ...
        sysDictTypeList.forEach(item -> {
            ...
            redisUtil.setEx(SysConf.REDIS_DICT_TYPE + SysConf.REDIS_SEGMENTATION + item.getDictType(), JsonUtils.objectToJson(result), 1, TimeUnit.DAYS);
        });
        return map;
    }

    @Override
    public Map<String, String> getDictDataLabelMapByType(String dictType) {

        //从Redis中获取内容
        String jsonResult = redisUtil.get(SysConf.REDIS_DICT_TYPE_LABEL + SysConf.REDIS_SEGMENTATION + dictType);
        //判断redis中是否有字典
        if (StringUtils.isNotEmpty(jsonResult)) {
            Map<String, String> map = (Map<String, String>) JsonUtils.jsonToMap(jsonResult, String.class);
            return map;
        }
        QueryWrapper<SysDictType> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(SQLConf.DICT_TYPE, dictType);
        ...
        sysDictDataQueryWrapper.orderByDesc(SQLConf.SORT, SQLConf.CREATE_TIME);
        List<SysDictData> list = sysDictDataService.list(sysDictDataQueryWrapper);
        Map<String, String> sysDictDataMap = new HashMap<>();
        for (SysDictData item : list) {
            sysDictDataMap.put(item.getDictValue(), item.getDictLabel());
        }

        // 设置过期时间
        redisUtil.setEx(SysConf.REDIS_DICT_TYPE_LABEL + SysConf.REDIS_SEGMENTATION + sysDictType.getDictType(), JsonUtils.objectToJson(sysDictDataMap), 1, TimeUnit.DAYS);
