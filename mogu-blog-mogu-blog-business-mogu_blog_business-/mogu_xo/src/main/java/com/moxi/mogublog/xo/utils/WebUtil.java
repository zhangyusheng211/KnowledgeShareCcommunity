...
public class WebUtil {
    ...
    public List<Map<String, Object>> getPictureMap(String result) {
        ...
        if (StringUtils.isEmpty(systemConfigJson)) {
            ...
            SystemConfig systemConfig = JsonUtils.jsonToPojo(systemConfigJson, SystemConfig.class);
            if (systemConfig == null) {
                log.error("[getPictureMap] 系统配置加载失败，请联系管理员");
                throw new QueryException("系统配置加载失败，请联系管理员");
            }
            picturePriority = systemConfig.getPicturePriority();
            localPictureBaseUrl = systemConfig.getLocalPictureBaseUrl();
            qiNiuPictureBaseUrl = systemConfig.getQiNiuPictureBaseUrl();
            ...
            if (picData.size() > 0) {
                for (int i = 0; i < picData.size(); i++) {
                    ...
                    if (EFilePriority.QI_NIU.equals(picturePriority)) {
                        ...
                        map.put(SysConf.M3U8_URL, qiNiuPictureBaseUrl + picData.get(i).get(SysConf.M3U8_URL));
                    } else if (EFilePriority.MINIO.equals(picturePriority)) {
                        map.put(SysConf.URL, minioPictureBaseUrl + picData.get(i).get(SysConf.MINIO_URL));
                        map.put(SysConf.M3U8_URL, minioPictureBaseUrl + picData.get(i).get(SysConf.M3U8_URL));
                    } else if (EFilePriority.ALIOSS.equals(picturePriority)) {
                        map.put(SysConf.URL, aliossPictureBaseUrl + picData.get(i).get(SysConf.ALIOSS_URL));
                        map.put(SysConf.M3U8_URL, aliossPictureBaseUrl + picData.get(i).get(SysConf.M3U8_URL));
                    } else {
                        map.put(SysConf.URL, localPictureBaseUrl + picData.get(i).get(SysConf.URL));
                        map.put(SysConf.M3U8_URL, localPictureBaseUrl + picData.get(i).get(SysConf.M3U8_URL));
                    }
    ...
    /**
     * List<Map<String,Object>> 转换为Map<String,Object>
     *
     * @param list
     * @return
     */
    public Map<String, String> listCovertMap(List<Map<String, String>> list) {
        Map<String, String> hashMap = new HashMap<>();
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).get("userid") != null && list.get(i).get("usercount") != null) {
                hashMap.put(list.get(i).get("userid"), String.valueOf(list.get(i).get("usercount")));
            }
        }
        return hashMap;
    }
