...
public class FileUtils {
    ...
    public static String getFileMd5(MultipartFile file) {
        ...
        try {
            ...
            log.error("获取文件MD5码异常：", e);
        }
        return getFileMd5(uploadBytes);
    }
     ...
     * @return
     */
    public static boolean isMiniFile(String fileName) {
        if (StringUtils.isBlank(fileName)) {
            return false;
        }
        String ext = FilenameUtils.getExtension(fileName).toLowerCase();

        List<String> miniFileList = new ArrayList<>();
        miniFileList.addAll(Arrays.asList(IMG_FILE));
        miniFileList.addAll(Arrays.asList(DOC_FILE));
        miniFileList.addAll(Arrays.asList(MUSIC_FILE));

        for (int i = 0; i < miniFileList.size(); i++) {
            if (miniFileList.get(i).equals(ext)) {
                return true;
            }
        }
        return false;
