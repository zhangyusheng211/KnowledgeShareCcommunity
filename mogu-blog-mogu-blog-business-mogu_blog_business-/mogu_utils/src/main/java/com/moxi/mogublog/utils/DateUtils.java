...
public class DateUtils {
     ...
     * @return
     */
    public static Date getNextDateTime(int day) {
        Date date = new Date(System.currentTimeMillis() - day * 24 * 60 * 60 * 1000L);
        return date;
    }
     ...
     * @return
     */
    public static Date getNextDate(Date beforeDate, int day) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(beforeDate);
        cal.add(Calendar.DAY_OF_MONTH, day);
        Date newDate = cal.getTime();
     ...
     * @return
     */
    public static boolean checkNowBetweenIn(Date startTime, Date endTime) {
        Date now = new Date();
     ...
     * @return
     */
    public static boolean crazyKFC() {
        // 创建一个Calendar实例
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(new Date());
        // 获取该时间的星期几（0表示周日，1表示周一，依此类推）
