	...
	<view class="template-circle tn-safe-area-inset-bottom">
		...
		<view class="tabs-fixed tn-bg-white" :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			<view class="tn-flex tn-flex-col-between tn-flex-col-center tn-padding-top-sm">
				<view class="justify-content-item" style="width: 95vw; overflow: hidden">
		...
		<view class="tn-flex tn-flex-wrap tn-margin-sm">
			<block v-for="(item, index) in dataList" :key="index">
				<view class="" style="width: 50%;" @click="tn('/pages/resource/resourceDetail?uid=' + item.uid)">
					<view class="tn-blogger-content__wrap" style="background-color: rgba(255,255,255,0.6);">
						<view class="image-picbook" style="background-repeat: no-repeat; background-position: center; "
							:style="'background-image:url(' + getFileIcon(item.resourceSort) + ')'">
						...
						<view class=" tn-padding-left-sm tn-padding-bottom-sm">
							<tn-tag v-if="item.chargeType == 1" :backgroundColor="colorMap['primary']" shape="radius"
								size="sm" fontColor="#FFFFFF">免费</tn-tag>
							<tn-tag v-if="item.chargeType == 2" :backgroundColor="colorMap['info']" shape="radius"
								size="sm" fontColor="#FFFFFF">付费</tn-tag>
							<tn-tag v-if="item.chargeType == 3" :backgroundColor="colorMap['success']" shape="radius"
								size="sm" fontColor="#FFFFFF">限时优惠</tn-tag>
							<tn-tag v-if="item.chargeType == 4" :backgroundColor="colorMap['warning']" shape="radius"
								size="sm" fontColor="#FFFFFF">会员免费</tn-tag>
						</view>
		...
		<tn-empty mode="list" v-if="dataList.length == 0"></tn-empty>
		<view v-else @click="loadMore" style="height: 50px;">
	...
	export default {
		...
		data() {
			return {
				...
				windowHeight: 0,
				//  文件图片Map映射
				fileIconList: [1, 2, 3, 4, 5],
				fileIconMap: {
					1: "https://oos.moguit.cn/mini/file/file_pdf.png",
					2: "https://oos.moguit.cn/mini/file/file_word.png",
					3: "https://oos.moguit.cn/mini/file/file_zip.png",
					4: "https://oos.moguit.cn/mini/file/file_video.png",

					unknown: "https://oos.moguit.cn/mini/file_unknown.png",
				},
				resourceSort: null, // 资源分类
			}
		},
		...
		methods: {
			...
			tabChange(index) {
				...
				switch (index) {
					...
					break;
					default: {
						let sortDataItem = this.sortData[index]
						console.log("sortData", sortDataItem)
						if (sortDataItem != null) {
							this.resourceSort = sortDataItem.fileSort
						}
					}
				}
				console.log("切换标签", index, this.resourceSort)
				this.getSubjectListMethod()
			},
			// tab选项卡切换
			topChange(index) {
			...
			getDictList: function() {
				let dictTypeList = ['sys_visit_auth', 'sys_resource_sort']
				getListByDictTypeList(dictTypeList).then(response => {
					if (response.code == this.$ECode.SUCCESS) {
						...
						for (let a = 0; a < this.resourceSortDictList.length; a++) {
							let item = {
								"name": this.resourceSortDictList[a].dictLabel,
								"fileSort": this.resourceSortDictList[a].dictValue
							}
							sortData.push(item)
						}
			...
			getSubjectListMethod() {
				...
				params.resourceSort = this.resourceSort
				params.orderByDescColumn = this.orderByDescColumn
				getResourceList(params).then((res) => {
					console.log('获取资源详情', res);
