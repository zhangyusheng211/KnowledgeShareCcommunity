      ...
      <view slot="back" class='tn-custom-nav-bar__back'
		...
		<view class="">
			<view class="tn-margin-top" :style="{paddingTop: vuex_custom_bar_height + 'px'}">
				<view class="tn-bg-white tn-margin tn-text-center" style="border-radius: 20rpx">
					<view class="tn-flex tn-flex-row-around tn-padding-sm tn-bg-blue--light"
						...
						<text>你已连续签到{{continuousDays}}天</text>
					</view>
					<view class="">
						<view class="tn-margin-top-xl tn-text-xl">当前用户积分</view>
						<view class="tn-text-bold tn-margin-top-sm tn-text-xxl">{{credits}}</view>
			...
			<view class="tn-flex tn-margin-xs">
				...
				<view v-if="this.vuex_custom_mini == '1'" class="tn-flex-1 screen-shadow tn-bg-white"
					style="margin: 30rpx 20rpx;padding: 40rpx 0;" @click="tn('/preferredPages/redeem')">
					<view class="tn-flex tn-flex-direction-column tn-flex-row-center tn-flex-col-center">
						<view
						...
						<view class="tn-text-center">
							<view class="tn-text-ellipsis tn-text-xl tn-color-purplered">资源兑换</view>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="tabbar footerfixed">
			<view class="action">
				...
				<view class="tn-color-gray">签到明细</view>
			</view>
			<view class="action" @click="userSignInMethod">
				...
				<view class="tn-color-gray">点击签到</view>
			</view>
			<view class="action">
				<view class="bar-icon">
...
<script>
	...
	export default {
		...
		created() {
			// 获取用户登录态
			this.getCurrentUserCreditsMethod(false)
			this.getSignDataByUserUidMethod(true)
		},
		methods: {
			// 跳转
			tn(e) {
			...
			// 获取用户签到记录
			getSignDataByUserUidMethod(refresh) {
				let params = {}
				params.refresh = refresh
				getSignDataByUserUid().then((res) => {
					console.log("获取用户签到记录", res)
					if (res.code == this.$ECode.SUCCESS) {
						// this.credits = res.data;
						this.isSignIn = res.data.todayIsSignIn
