					...
					<view class="tn-flex">
						<view class="tn-margin-sm tn-padding-top-xs" style="width: 100%">
							<view class="tn-text-lg tn-text-bold clamp-text-1 tn-text-justify">
								...
								<!--构建标题-->
								<view v-if="item.collectType == '1' && item.blog">
									<text class=""
										@click="tn('/pages/article/details?oid=' + item.blog.oid)">{{ item.blog.title }}</text>
									<view style="float: right;">
										<tn-tag size="sm" shape="radius" backgroundColor="tn-bg-blue"
											fontColor="tn-color-white">博客
										</tn-tag>
									</view>
								</view>

								<view v-if="item.collectType == '2' && item.question">
									<text class="">{{ item.question.title }}</text>
									<view style="float: right;">
										<tn-tag size="sm" shape="radius" backgroundColor="tn-bg-blue"
											fontColor="tn-color-white">问答
										</tn-tag>
									</view>
								</view>

								<view v-if="item.collectType == '4' && item.userMoment">
									<text class=""
										@click="tn('/pages/moment/details?uid=' + item.userMoment.uid)">用户动态</text>
									<view style="float: right;">
										<tn-tag size="sm" shape="radius" backgroundColor="tn-bg-blue"
											fontColor="tn-color-white">动态
										</tn-tag>
									</view>
								</view>

								<view v-if="item.collectType == '6' && item.problem">
									<text class="">{{ item.problem.title }}</text>
									<view style="float: right;">
										<tn-tag size="sm" shape="radius" backgroundColor="tn-bg-blue"
											fontColor="tn-color-white">问题
										</tn-tag>
									</view>
								</view>

								<view v-if="item.collectType == '11' && item.resource">
									<text class="">{{ item.resource.name }}</text>
									<view style="float: right;">
										<tn-tag size="sm" shape="radius" backgroundColor="tn-bg-blue"
											fontColor="tn-color-white">资源
										</tn-tag>
									</view>
								</view>
							</view>
							...
							<!--构建详情-->
							<view v-if="item.collectType == '1' && item.blog" class="tn-padding-top-xs tn-flex"
								style="height: 122rpx" @click="tn('/pages/article/details?oid=' + item.blog.oid)">
								<text class="tn-text-df tn-color-gray clamp-text-2 tn-text-justify">
									{{ item.blog.summary.replace(/[\r\n]+/g, " ") }}
								</text>
							</view>

							<view v-if="item.collectType == '2' && item.question" class="tn-padding-top-xs tn-flex"
								style="height: 122rpx">
								<text class="tn-text-df tn-color-gray clamp-text-2 tn-text-justify">
									{{ item.question.summary.replace(/[\r\n]+/g, " ") }}
								</text>
							</view>

							<view v-if="item.collectType == '4' && item.userMoment" class="tn-padding-top-xs tn-flex"
								style="height: 122rpx">
								<text class="tn-text-df tn-color-gray clamp-text-2 tn-text-justify">
									{{ item.userMoment.content.replace(/[\r\n]+/g, " ") }}
								</text>
							</view>

							<view v-if="item.collectType == '6' && item.problem" class="tn-padding-top-xs tn-flex"
								style="height: 122rpx">
								<text class="tn-text-df tn-color-gray clamp-text-2 tn-text-justify">
									{{ item.problem.summary.replace(/[\r\n]+/g, " ") }}
								</text>
							</view>
							<view v-if="item.collectType == '11' && item.resource" class="tn-padding-top-xs tn-flex"
								style="height: 122rpx">
								<text class="tn-text-df tn-color-gray clamp-text-2 tn-text-justify">
									{{ item.resource.summary.replace(/[\r\n]+/g, " ") }}
								</text>
							</view>
