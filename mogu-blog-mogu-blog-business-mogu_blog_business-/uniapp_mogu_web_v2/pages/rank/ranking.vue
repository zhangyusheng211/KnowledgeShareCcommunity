<template>
	<view class="template-ranking tn-safe-area-inset-bottom">
		...
		<view class="tn-flex tn-flex-row-around">
			<view class="user-info__container justify-content-item" v-if="creditsTopList[1]">
				<view class="user-info__avatar-two tn-flex-col-center tn-flex-row-center">
					<view class="tn-shadow-blur" :style="{ backgroundImage: `url(${creditsTopList[1].photoUrl})` }"
						style="width: 140rpx;height: 140rpx;background-size: cover;">
					</view>
				</view>
				<view class="user-info__nick-name-two clamp-text-1">{{creditsTopList[1].nickName}}</view>
				<view class="user-info__nick-number-two clamp-text-1" style="margin-left: -8rpx;">
					<text class="tn-icon-data tn-padding-right-xs"></text> {{creditsTopList[1].credits}}
				</view>
			</view>
			<view class="user-info__container justify-content-item" v-if="creditsTopList[0]">
				<view class="user-info__avatar-one tn-flex-col-center tn-flex-row-center">
					<view class="tn-shadow-blur" :style="{ backgroundImage: `url(${creditsTopList[0].photoUrl})` }"
						style="bwidth: 180rpx;height: 180rpx;background-size: cover;">
					</view>
				</view>
				<view class="user-info__nick-name-one clamp-text-1">{{creditsTopList[0].nickName}}</view>
				<view class="user-info__nick-number-one clamp-text-1" style="margin-left: -8rpx;">
					<text class="tn-icon-data tn-padding-right-xs"></text> {{creditsTopList[0].credits}}
				</view>
			</view>
			<view class="user-info__container justify-content-item" v-if="creditsTopList[2]">
				<view class="user-info__avatar-three tn-flex-col-center tn-flex-row-center">
					<view class="tn-shadow-blur" :style="{ backgroundImage: `url(${creditsTopList[2].photoUrl})` }"
						style="width: 120rpx;height: 120rpx;background-size: cover;">
					</view>
				</view>
				<view class="user-info__nick-name-three clamp-text-1">{{creditsTopList[2].nickName}}</view>
				<view class="user-info__nick-number-three clamp-text-1" style="margin-left: -8rpx;">
					<text class="tn-icon-data tn-padding-right-xs"></text> {{creditsTopList[2].credits}}
				</view>
			</view>
		</view>
			...
			<view class="" style="padding-top: 20rpx;">
				<view class="nav_title--wrap">
					<view class="nav_title tn-cool-bg-color-15">
						<text class="tn-icon-data tn-padding-right-sm tn-text-xxl"></text>
						<text class="tn-text-xl">积分榜单 · 社区排行</text>
						<text class="tn-icon-data tn-padding-left-sm tn-text-xxl"></text>
					</view>
				</view>
			</view>
			<block v-for="(item,index) in creditsTopList" v-if="index >= 3" :key="item.uid">
						...
						<view class="tn-flex tn-flex-row-center tn-flex-col-center">
							<view class="tn-flex tn-flex-row-center tn-padding-right">
								<text class="tn-text-bold tn-text-xxl"
									style="color: #B0B7C6;">{{ item.userNumber }}</text>
							</view>
							<view class="tn-flex tn-flex-row-center tn-flex-col-center">
								<!-- <tn-avatar
                  class=""
                  ...
                  :src="item.userAvatar">
								...
								<view class="avatar-all">
									<view class="tn-shadow-blur"
										:style="'background-image:url('+ item.photoUrl + ');width: 80rpx;height: 80rpx;background-size: cover;'">
									</view>
								</view>
					...
					<view class="justify-content-item tn-flex-row-center tn-margin-top">
						<text class="tn-text-xl tn-padding-right">{{ item.credits }}</text>
						<text class="tn-icon-data icon-ranking"></text>
					</view>
				</view>
			</block>
		</view>
				...
				<view class="justify-content-item tn-margin-top">
					<view class="tn-flex tn-flex-row-center tn-flex-col-center">
						...
						<view class="tn-flex tn-flex-row-center tn-flex-col-center">
							<view class="avatar-all">
								<view class="tn-shadow-blur"
									style="background-image:url('https://tnuiimage.tnkjapp.com/blogger/blogger_beibei.jpg');width: 80rpx;height: 80rpx;background-size: cover;">
								</view>
							</view>
							<view class="tn-padding-right tn-text-ellipsis">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取用户积分
			getCreditsTopListMethod() {
				let params = {}
				params.pageSize = this.pageSize
				params.currentPage = this.currentPage
				getLeaderAll(params).then((res) => {
					console.log("获取积分排行", res)
					if (res.code == this.$ECode.SUCCESS) {
						this.creditsTopList = res.data
					}
				})
			},
		}
