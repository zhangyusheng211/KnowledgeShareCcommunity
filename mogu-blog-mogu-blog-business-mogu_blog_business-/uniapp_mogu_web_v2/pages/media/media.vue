			...
			<view class="tn-padding-bottom-lg tn-padding-top-sm">
				<block v-for="(item,index) in dataList" :key="index" class="">
					<view class="tn-blogger-content__wrap tn-radius"
						@click="tn('/pages/media/mediaDetail?uid=' + item.uid)"
						...
						<view class="tn-margin-top-sm" style="height: 320rpx; position: relative">
							...
							<image
								...
								v-if="item.images" :src="item.images" mode="aspectFill">
							</image>
							<image
								style="width: 100%; height: 320rpx;  border-top-left-radius: 18rpx; border-top-right-radius: 18rpx;"
								class="tn-blogger-content__main-image tn-blogger-content__main-image--1 tn-margin-bottom"
							...
							<view v-if="!item.images " class="carousel-title">
								<text class="tn-blogger-content__label__desc tn-text-bold"
									style="position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); color: #FFFFFF; font-size: 22px; line-height: 30px;">{{ item.title }}</text>
							</view>

						</view>
						<view class="tn-blogger-content__label tn-text-center tn-margin-top-sm">
							<text
								class="tn-blogger-content__label__desc tn-text-lg tn-text-bold">{{ item.title }}</text>
						</view>
						<view class="tn-text-center tn-margin-top-sm tn-padding-bottom-sm">
							...
							<tn-tag margin="5rpx" class="tn-margin-right" :backgroundColor="colorMap['success']"
								shape="radius" fontColor="#FFFFFF" v-if="item.price > 0">付费</tn-tag>
							<tn-tag margin="5rpx" class="tn-margin-right" :backgroundColor="colorMap['success']"
								shape="radius" fontColor="#FFFFFF" v-if="item.price ==  0">免费</tn-tag>

							<tn-tag margin="5rpx" v-if="item.subjectSort && item.subjectSort.sort"
							...
							<tn-tag margin="5rpx" v-if="item.subjectSort && item.subjectSort.sort"
								:backgroundColor="colorList[item.subjectSort.sort % colorList.length]" shape="radius"
								plain="true" :width="200"
								:fontColor="colorList[item.subjectSort.sort % colorList.length]">{{item.clickCount}}
								人学习</tn-tag>
		...
		methods: {
			...
			getCategoryList() {
				listCategory({
					...
					console.log("加载课程列表", response)
					let sortData = this.sortData
					let categoryOptions = response.data.records;
					for (let a = 0; a < categoryOptions.length; a++) {
						let item = {
							"name": categoryOptions[a].name,
							"uid": categoryOptions[a].uid
						}
						sortData.push(item)
					}
			...
			tabChange(index) {
				...
				switch (index) {
					...
					default: {
						...
						if (subjectSort != null) {
							this.categoryId = subjectSort.uid
						}
					}
				}
				this.getSubjectListMethod()
