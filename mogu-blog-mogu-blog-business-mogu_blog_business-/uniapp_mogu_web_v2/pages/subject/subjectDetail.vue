	...
	<view class="template-circle tn-safe-area-inset-bottom">
				...
				<view class="tn-blogger-content__wrap tn-radius"
					...
					<view class="tn-margin-top-sm" style="height: 320rpx; position: relative">
						...
						<image
							...
							v-if="subject.photoList && subject.photoList[0]" :src="subject.photoList[0]"
							mode="aspectFill">
						</image>
						<image
							style="width: 100%; height: 320rpx;  border-top-left-radius: 18rpx; border-top-right-radius: 18rpx;"
						...
						<view v-if="subject.openPictureTitle === 1" class="carousel-title">
							<text class="tn-blogger-content__label__desc tn-text-bold"
								style="position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); color: #FFFFFF; font-size: 22px;">{{ subject.subjectName }}</text>
						</view>

					</view>
					<view class="tn-blogger-content__label tn-text-center tn-margin-top-sm">
						<text
							class="tn-blogger-content__label__desc tn-text-lg tn-text-bold">{{ subject.subjectName }}</text>
					</view>
					<view class="tn-text-center tn-margin-top-sm tn-padding-bottom-sm">
						<tn-tag margin="5rpx" class="tn-margin-right" backgroundColor="#e6a23c" shape="radius"
							fontColor="#FFFFFF">精选</tn-tag>

						<tn-tag margin="5rpx" v-if="subject.subjectSort" :plain="true"
							:backgroundColor="colorList[subject.subjectSort.sort % colorList.length]" shape="radius"
							:fontColor="colorList[subject.subjectSort.sort % colorList.length]">{{subject.subjectSort.name}}</tn-tag>

						<!--区分付费和免费-->
						<tn-tag v-for="tagItem in visitAuthDictList" :key="tagItem.uid"
							v-if="parseInt(tagItem.dictValue) === subject.visitAuth" margin="5rpx"
							style="margin-left: 10rpx;" :plain="true"
							:backgroundColor="colorMap[tagItem.listClass]? colorMap[tagItem.listClass]:'#01BEFF'"
							shape="radius"
							:fontColor="colorMap[tagItem.listClass]? colorMap[tagItem.listClass]:'#01BEFF'">{{ [5, 6, 10, 11, 12, 13].includes(subject.visitAuth)?tagItem.dictLabel:'免费'}}</tn-tag>
		...
		<view class="tn-padding">
			...
			<view
				...
				v-if="current == 0">
				<mp-html v-if="subject.content" :content="subject.content"></mp-html>
				<tn-empty mode="data" v-else></tn-empty>
			</view>

			<view class="tn-padding-bottom-lg" v-else>
				<view v-for="(item, index) in dataList" :key="item.uid" v-if="item.blog"
					@click="tn('/pages/article/details?oid=' + item.blog.oid)">
					<view style="height: 90rpx; line-height: 90rpx;">
						<text class="tn-blogger-content__label__desc"
							style="text-overflow: ellipsis;white-space: nowrap;overflow: hidden;">({{index + 1}}).{{ item.blog.title }}</text>
	...
	export default {
		...
		onShareTimeline: function() {
			return {
				...
				path: '/pages/subject/subjectDetail?uid=' + this.subjectUid,
			}
		},
		onReady() {
			...
			getSubjectListMethod() {
				...
				getSubjectList(params).then((res) => {
					...
					if (res.code == this.$ECode.SUCCESS) {
						let subjectList = res.data.records;
						// 获取专栏
