...
//    private XfXhConfig xfXhConfig;
     ...
     */
    public WebSocket sendMsg(XfXhConfig xfXhConfig, String uid, List<MsgDTO> msgList, WebSocketListener listener) {
        // 获取鉴权url
        String authUrl = this.getAuthUrl(xfXhConfig);
        // 鉴权方法生成失败，直接返回 null
        if (authUrl == null) {
            return null;
        }
        ...
        // 组装请求参数
        RequestDTO requestDTO = getRequestParam(xfXhConfig, uid, msgList);
        // 发送请求
     ...
     * @return 鉴权访问大模型的路径
     */
    public String getAuthUrl(XfXhConfig xfXhConfig) {
        try {
            ...
            // 拼接
            String authorization = String.format("api_key=\"%s\", algorithm=\"%s\", headers=\"%s\", signature=\"%s\"", xfXhConfig.getApiKey(), "hmac-sha256", "host date request-line", sha);
            // 拼接地址
     ...
     */
    public RequestDTO getRequestParam(XfXhConfig xfXhConfig, String uid, List<MsgDTO> msgList) {
