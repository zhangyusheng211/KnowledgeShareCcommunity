...
public class RabbitMqConfig {
    ...
    public static final String ROUTING_KEY_DOMAIN_EVENT = "domain.event";
     ...
     */
    @Bean(DOMAIN_EVENT)
    public Queue MOGU_TASK() {
        return new Queue(DOMAIN_EVENT);
    }
    ...
    /**
     * domain.event 队列绑定交换机，指定routingKey
     *
     * @param queue
     * @param exchange
     * @return
     */
    @Bean
    public Binding BINDING_QUEUE_INFORM_DOMAIN_EVENT(@Qualifier(DOMAIN_EVENT) Queue queue, @Qualifier(EXCHANGE_DIRECT) Exchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with(ROUTING_KEY_DOMAIN_EVENT).noargs();
    }
