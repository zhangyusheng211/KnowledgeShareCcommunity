<template>
    <div class="app-container">
        ...
        <div class="filter-container" style="margin: 10px 0 10px 0;" v-permission="'/link/getList'">
            ...
            <el-select v-model="queryParams.isPublish" clearable placeholder="是否发布" style="width:140px" size="small">
                <el-option
...
<!--            </el-table-column>-->
            ...
            <el-table-column label="分类名称" width="200" align="center">
                <template slot-scope="scope">
                    <span>{{ setMask(scope.row.name) }}</span>
                </template>
            </el-table-column>

            <el-table-column label="分类简介" width="200" align="center">
                <template slot-scope="scope">
                    <span>{{ setMask(scope.row.summary) }}</span>
                </template>
            </el-table-column>
        ...
        <el-dialog :title="title" :visible.sync="dialogFormVisible">
            <el-form :model="form" :rules="rules" ref="form">
                ...
                <el-form-item label="分类名称" :label-width="formLabelWidth" prop="bizId">
                    <el-input v-model="form.name" auto-complete="off"></el-input>
                </el-form-item>

                <el-form-item label="分类简介" :label-width="formLabelWidth" prop="secretKey">
...
<script>
import {addCommonCategory, deleteBatchCommonCategory, editCommonCategory, getCommonCategoryList} from "@/api/commonCategory";
...
export default {
    ...
    methods: {
        ...
        handleDelete: function (row) {
            let that = this;
            this.$confirm("此操作将把密钥分类删除, 是否继续?", "提示", {
                ...
                .then(() => {
                    let params = {};
                    params.uid = row.uid;
                    deleteBatchCommonCategory(params).then(response => {
                        if (response.code === this.$ECode.SUCCESS) {
                            this.$commonUtil.message.success(response.message)
                            that.commonCategoryList();
                        } else {
        ...
        submitForm: function () {
            this.form.bizType = this.bizType
            this.$refs.form.validate((valid) => {
                if (!valid) {
                    console.log("校验失败")
                } else {
                    if (this.isEditForm) {
                        editCommonCategory(this.form).then(response => {
                            if (response.code == this.$ECode.SUCCESS) {
                                this.$commonUtil.message.success(response.message)
                        ...
                        addCommonCategory(this.form).then(response => {
                            if (response.code == this.$ECode.SUCCESS) {
                                this.$commonUtil.message.success(response.message)
                                this.dialogFormVisible = false;
                                this.commonCategoryList();
