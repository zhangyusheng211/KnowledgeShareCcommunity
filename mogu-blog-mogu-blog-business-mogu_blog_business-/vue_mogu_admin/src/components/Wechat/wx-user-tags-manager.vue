...
import {deleteWxUserTags, getWxUserTags, saveWxUserTags} from "../../api/wechat";
export default {
    ...
    methods: {
        ...
        deleteTag(tagid) {
            ...
            this.$confirm(`确定删除标签?`, '提示', {
                ...
                deleteWxUserTags(tagid).then(data=> {
                    if (data && data.code === this.$ECode.SUCCESS) {
                        this.getWxUserTags();
                        this.$emit('change');
                    } else {
        ...
        saveTag(name,tagid){
            ...
            this.submitting=true
            let params = {}
            params.id = tagid?tagid:undefined
            params.name = name
            saveWxUserTags().then(data=> {
                if (data && data.code === this.$ECode.SUCCESS) {
                    this.getWxUserTags();
                    this.$emit('change');
                } else {
