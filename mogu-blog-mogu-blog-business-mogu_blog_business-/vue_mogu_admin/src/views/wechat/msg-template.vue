<template>
    <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" style="margin: 10px">
            <el-form-item>
                <el-input size="small" v-model="dataForm.title" placeholder="标题" clearable></el-input>
            </el-form-item>
            <el-form-item>
                <el-button size="small" @click="getDataList()">查询</el-button>
                <el-button  size="small" type="success" @click="copyHandle()" :disabled="dataListSelections.length <= 0">批量复制</el-button>
                <el-button size="small" type="success" @click="templateMsgTaskHandle()" :disabled="dataListSelections.length!=1">推送消息</el-button>
                <el-button size="small" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
            </el-form-item>
            <el-form-item class="fr">
                <el-button size="small" icon="el-icon-sort" type="success" @click="syncWxTemplate()" :disabled="synchonizingWxTemplate">{{synchonizingWxTemplate?'同步中...':'同步公众号模板'}}</el-button>
                <el-button size="small"><el-link type="primary" icon="el-icon-link" target="_blank" href="https://kf.qq.com/faq/170209E3InyI170209nIF7RJ.html">模板管理指引</el-link></el-button>
            </el-form-item>
        </el-form>
...
import TemplateMsgTask from '@/components/Wechat/template-msg-task'
import {deleteWxMsgTemplate, getWxMsgTemplate, syncWxMsgTemplate, saveWxMsgTemplate} from "../../api/wechat";
export default {
    data() {
        return {
            dataForm: {
    ...
    methods: {
        ...
        deleteHandle(id) {
            ...
            this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
                ...
                deleteWxMsgTemplate(ids).then(data => {
                    if (data && data.code === this.$ECode.SUCCESS) {
                        this.$message({
                            message: '操作成功',
                            type: 'success',
        ...
        addMsgTemplate(msgTemplate){
            ...
            saveWxMsgTemplate(msgTemplate).then(data => {
                this.synchonizingWxTemplate=false
                if (data && data.code === this.$ECode.SUCCESS) {
                    this.$message({
                        message: '同步完成',
