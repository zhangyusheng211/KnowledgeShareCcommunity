...
import {deleteWxMsgReplyRules, getWxMsgReplyRules} from "../../api/wechat";
export default {
    components: {
        AddOrUpdate
    },
    ...
    methods: {
        ...
        getDataList() {
            ...
            params.matchValue = this.dataForm.matchValue
            getWxMsgReplyRules(params).then(data => {
        ...
        deleteHandle(id) {
            ...
            this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
                ...
                deleteWxMsgReplyRules(ids).then(data => {
                    if (data && data.code === this.$ECode.SUCCESS) {
                        this.$message({
                            message: '操作成功',
                            type: 'success',
