<template>
    <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" style="margin: 10px">
            <el-form-item>
                <el-select size="small" v-model="dataForm.startTime" placeholder="时间">
                    <el-option v-for="(name,key) in timeSelections" :key="key" :value="name" :label="key"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
                <el-select size="small" v-model="dataForm.msgTypes" placeholder="消息类型">
...
<script>
import {getWxMsgList, replyWxMsg, listByIdsWxUsers} from "../../api/wechat";
...
export default {
    ...
    components: {
        ...
        WxMsgPreview:()=>import('@/components/Wechat/wx-msg-preview')
    },
    created() {
        this.getDataList()
    },
    methods: {
        ...
        getDataList() {
            ...
            params.msgTypes = this.msgTypes
            params.startTime = this.startTime
            params.sidx = 'create_time'
            params.order = 'desc'
            getWxMsgList().then(data => {
        ...
        refreshUserList(msgList){
            ...
            listByIdsWxUsers(openidList).then(data => {
                if (data && data.code === this.$ECode.SUCCESS) {
                    this.userDataList = this.userDataList.concat(data.data)
                }
            })
