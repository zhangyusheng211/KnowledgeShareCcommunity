<template>
    <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" style="margin: 10px">
            <el-form-item>
                <el-select size="small" v-model="dataForm.tagid" filterable clearable placeholder="用户标签">
                    <el-option v-for="item in wxUserTags" :key="item.id" :label="item.name" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
                <el-input size="small" v-model="dataForm.nickname" placeholder="昵称" clearable></el-input>
            </el-form-item>
            <el-form-item>
                <el-input size="small" v-model="dataForm.city" placeholder="城市" clearable></el-input>
            </el-form-item>
            <el-form-item>
                <el-input size="small" v-model="dataForm.qrSceneStr" placeholder="关注场景值" clearable></el-input>
            </el-form-item>
            <el-form-item>
                <el-button size="small" type="primary" @click="getDataList()">查询</el-button>
                <el-button size="small" type="primary" @click="$refs.wxUserTagging.init('tagging')" :disabled="dataListSelections.length <= 0">绑定标签</el-button>
                <el-button size="small" type="primary" @click="$refs.wxUserTagging.init('untagging')" :disabled="dataListSelections.length <= 0">解绑标签</el-button>
                <el-button size="small" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
            </el-form-item>
            <el-form-item class="fr">
                <el-button size="small" icon="el-icon-price-tag" type="success" @click="$refs.wxUserTagsEditor.show()">标签管理</el-button>
                <el-button size="small" icon="el-icon-sort" type="success" @click="syncWxUsers()">同步粉丝</el-button>
            </el-form-item>
        </el-form>
...
<script>
import WxUserTagsManager from '@/components/Wechat/wx-user-tags-manager'
import WxUserTagging from './wx-user-tagging'
import { mapState } from 'vuex'
import {deleteWxUser, getWxUserList, syncWxUsers} from "../../api/wechat";
export default {
    data() {
        return {
            dataForm: {
    ...
    methods: {
        ...
        getDataList() {
            ...
            params.tagid = this.dataForm.tagid
            params.city = this.dataForm.city
            params.qrSceneStr = this.dataForm.qrSceneStr
            params.sidx = 'subscribe_time'
            params.order = 'desc'
            getWxUserList(params).then(data => {
                console.log("获取粉丝列表", data)
                if (data && data.code === this.$ECode.SUCCESS) {
