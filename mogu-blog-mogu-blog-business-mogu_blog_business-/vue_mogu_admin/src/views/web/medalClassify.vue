    ...
    <div class="app-container">
        ...
        <div class="filter-container" style="margin: 10px 0 10px 0;" v-permission="'/tag/getList'">
            ...
            <el-button class="filter-item" type="primary" @click="handleAdd" icon="el-icon-edit" size="small"
                       v-permission="'/questionTag/add'">添加勋章分类
            </el-button>
        ...
        <el-table :data="tableData"
            ...
            <el-table-column label="类型" width="100" align="center">
                <template slot-scope="scope">
                    <el-tag
                        v-for="item in medalTypeDictList"
                        v-if="scope.row.type == parseInt(item.dictValue)"
                        :key="item.uid"
                        :type="item.listClass"
                    >{{ item.dictLabel }}
                    </el-tag>
        ...
        <el-dialog :title="title" :visible.sync="dialogFormVisible">
            <el-form :model="form" :rules="rules" ref="form">
                ...
                <el-form-item label="勋章分类名" :label-width="formLabelWidth" prop="name">
...
<script>
import {addMedalClassify, deleteBatchMedalClassify, editMedalClassify, getMedalClassifyList} from "@/api/medalClassify";
...
export default {
    ...
    methods: {
        ...
        getDictList: function () {
            let dictTypeList = ['sys_publish_status', 'sys_medal_type']
        ...
        handleDelete: function (row) {
            var that = this;
            this.$confirm("此操作将把勋章分类删除, 是否继续?", "提示", {
                ...
                .then(() => {
                    var params = [];
                    params.push(row);
                    deleteBatchMedalClassify(params).then(response => {
                        if (response.code == this.$ECode.SUCCESS) {
        ...
        handleDeleteBatch: function () {
            ...
            this.$confirm("此操作将把选中的勋章分类删除, 是否继续?", "提示", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning"
            })
                .then(() => {
                    deleteBatchMedalClassify(that.multipleSelection).then(response => {
                        if (response.code == this.$ECode.SUCCESS) {
                            this.$commonUtil.message.success(response.message)
                        } else {
                            this.$commonUtil.message.error(response.message)
        ...
        submitForm: function () {
            this.$refs.form.validate((valid) => {
                if (!valid) {
                    ...
                    if (this.isEditForm) {
                        editMedalClassify(this.form).then(response => {
                            if (response.code == this.$ECode.SUCCESS) {
                                this.$commonUtil.message.success(response.message)
                        ...
                        addMedalClassify(this.form).then(response => {
