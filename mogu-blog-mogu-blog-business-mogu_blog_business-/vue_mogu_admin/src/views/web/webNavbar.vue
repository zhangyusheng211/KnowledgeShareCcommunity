    ...
    <div class="app-container">
        ...
        <div class="filter-container" style="margin: 10px 0 10px 0;">
            <el-button class="filter-item" type="primary" @click="handleAdd" icon="el-icon-edit" size="small"
                       v-permission="'/categoryMenu/add'">添加导航栏
            </el-button>
        </div>
        ...
        <el-table :data="tableData" style="width: 100%">
                    ...
                    <el-form label-position="left" inline class="demo-table-expand">
                        ...
                        <el-table :data="scope.row.childWebNavbar" :show-header="showHeader" style="width: 100%">
                            ...
                            <el-table-column label width="100" align="center">
                                <template slot-scope="scope_child">
                                    <el-tag v-for="item in menuLevelDictList" :key="item.uid"
                                            v-if="scope_child.row.navbarLevel  == item.dictValue"
                            ...
                            <el-table-column width="100" align="center">
                                <template slot-scope="scope">
                                    <el-tag v-for="item in yesNoDictList" :key="item.uid"
                                            v-if="scope.row.isShow == item.dictValue" :type="item.listClass">
                                        {{ item.dictLabel }}
                                    </el-tag>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column width="120" align="center">
                                <template slot-scope="scope">
                                    <el-tag v-for="item in jumpExternalDictList" :key="item.uid"
                                            v-if="scope.row.isJumpExternalUrl == item.dictValue" :type="item.listClass">
                                        {{ item.dictLabel }}
                                    </el-tag>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column fixed="right" min-width="230">
                                <template slot-scope="scope_child">
                                    <el-button @click="handleEdit(scope_child.row)" type="primary" size="small"
                                               v-permission="'/webNavbar/edit'">编辑
                                    </el-button>
                                    <el-button @click="handleDelete(scope_child.row)" type="danger" size="small"
                                               v-permission="'/webNavbar/delete'">删除
                                    </el-button>
                                </template>
                            </el-table-column>
                        </el-table>
            ...
            <el-table-column label="导航栏级别" width="100" align="center">
                <template slot-scope="scope">
                    <el-tag v-for="item in menuLevelDictList" :key="item.uid"
                            v-if="scope.row.navbarLevel  == item.dictValue" :type="item.listClass">{{ item.dictLabel }}
                    </el-tag>
                </template>
            </el-table-column>
            ...
            <el-table-column label="字体颜色" width="100" align="center">
                <template slot-scope="scope">
            ...
            <el-form :model="form" :rules="rules" ref="form">
                ...
                <el-form-item label="是否跳转外链" :label-width="formLabelWidth" prop="isShow">
                    <el-radio-group v-model="form.isJumpExternalUrl" size="small">
                        <el-radio v-for="item in jumpExternalDictList" :key="item.uid" :label="parseInt(item.dictValue)"
                ...
                <el-form-item label="是否显示" :label-width="formLabelWidth" prop="isShow">
                    <el-radio-group v-model="form.isShow" size="small">
                        <el-radio v-for="item in yesNoDictList" :key="item.uid" :label="parseInt(item.dictValue)"
                                  border>{{ item.dictLabel }}
                        </el-radio>
                    </el-radio-group>
                </el-form-item>

                <el-form-item label="标题颜色" :label-width="formLabelWidth" prop="sort">
...
<script>
import {addWebNavbar, deleteWebNavbar, editWebNavbar, getWebNavbarAllList} from "@/api/webNavbar";
