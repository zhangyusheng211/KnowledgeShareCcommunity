...
public class LoggerAspect {
    ...
    private void handle(ProceedingJoinPoint point) throws Exception {
        ...
        log.info("[LoggerAspect] {} | {} - {} {} - {}", bussinessName, IpUtils.getIpAddr(request), RequestUtil.getMethod(), RequestUtil.getRequestUrl(), ua);
        if (!save) {
            log.info("[LoggerAspect] 日志不落库");
            return;
        }

        // 获取参数名称和值
        Map<String, Object> nameAndArgsMap = AopUtils.getFieldsName(point);
        log.info("[LoggerAspect] Request: {}", JsonUtils.objectToJson(nameAndArgsMap));
        ...
        if (result != null) {
            ...
            threadPoolTaskExecutor.execute(
                    new SysLogHandle(userUid, ip, os, browser,
                            ...
                            result.get(SysConf.OTHER_DATA), redisUtil, null));
        }
    }
}
