...
public class CreatCodeRestApi {
    ...
    public String validCode(@ApiParam(name = "info", value = "注册手机号码或邮箱", required = true) @RequestBody(required = true) String info) {

        if (!CheckUtils.checkMobileNumber(info)) {
            if (!CheckUtils.checkEmail(info)) {
                return ResultUtil.errorWithMessage("输入的邮箱或手机号格式有误");
            }
        }
        //生成6位随机数
